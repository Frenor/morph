...
export function CategoryTable({
  ...
  const maxValueLength = categories.reduce(
    (max, category) => Math.max(max, category.values.length),
    0
  );
  ...
  return (
    <table className="category-table">
      ...
      <tbody>
        {[...Array(maxValueLength)].map((_, valueIndex) => (
          <tr key={valueIndex}>
            {categories.map((category, categoryIndex) => {
              ...
              return value ? (
                <td
                  ...
                  onMouseEnter={() =>
                    onHoverExclude && setHoverExclusions(onHoverExclude(value))
                  }
                  onMouseLeave={() => setHoverExclusions([])}
                >
                  {value.name}
                </td>
              ) : (
                <td key={categoryIndex + "-" + valueIndex}></td>
              );
            })}
          </tr>
        ))}
